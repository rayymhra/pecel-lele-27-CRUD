...
$totalRevenue = ($conn->query("SELECT IFNULL(SUM(total),0) AS revenue FROM orders WHERE status = 'selesai'")->fetch_assoc())['revenue'] ?? 0;

$totalMenus = ($conn->query("SELECT COUNT(*) AS total FROM menu")->fetch_assoc())['total'] ?? 0;
...
<div class="container mt-4">
    <h1 class="mb-4 dashboard-title">Dashboard Admin</h1>
    ...
    <div class="row g-4">
        ...
        <div class="col-md-3">
            <div class="card card-yellow p-3">
                ...
                <div class="dashboard-value">Rp<?= number_format($totalRevenue, 0, ',', '.') ?></div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card card-yellow p-3">
                <h6>Total Menu</h6>
                <div class="dashboard-value"><?= number_format($totalMenus) ?></div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card card-yellow p-3">
                <h6>Total User</h6>
                <div class="dashboard-value"><?= number_format($totalUsers) ?></div>
            </div>
        </div>
    </div>
    ...
    <!-- <div class="mt-5 quick-link">
        ...
        <a href="menu/index.php" class="btn btn-warning me-2">Kelola Menu</a>
        <a href="users/index.php" class="btn btn-warning">Kelola User</a>
    </div> -->
</div>
